...
//Toggle sidebar navigation links
...
//                let index = entry.target.id.split("-")[1]; // Extract index from ID

//                sidebarLinks.forEach(link => link.classList.remove("active"));
//                document.getElementById(`nav-${index}`).classList.add("active");
//            }
//        });
//    }, { threshold: 0.5 }); // Adjust threshold for accuracy

//    sections.forEach(section => observer.observe(section));
//});
// auto scroll left sidebar links
document.addEventListener("DOMContentLoaded", function () {
    let sections = document.querySelectorAll(".sequence-title");
    let sidebarLinks = document.querySelectorAll(".sequence-sidebar-link");
